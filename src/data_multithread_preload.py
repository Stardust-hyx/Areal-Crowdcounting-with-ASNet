...
class PreloadData:
    def __init__(self, image_path, is_preload=False, is_label=False, is_mask=False, is_transfrom=False, is_transform_in_gray=False, img_size=(640, 640)):
        ...
        self.min_available_memory = 1 * 1024 * 1024 * 1024  # GB

        # make path for pickle
        time_now = datetime.datetime.now()
        self.pickle_path = os.path.join(r'E:/ASNet/pickle', '%4d%02d%02d%02d%02d%02d%06d_%s' %
        ...
        for filename in self.image_filename_list:
            if self.is_preload:
                if psutil.virtual_memory().available > self.min_available_memory:
                    index += 1
                    self.preload_data_dict[filename] = self.read_blob(filename, self.img_size)
                    if index % 100 == 0:
    ...
    def get_blob_by_index(self, index):
        ...
        if this_blob is None:  # no data is preloaded for this blob
            ...
            if os.path.isfile(pickle_file_path):
                ...
                this_blob = self.read_blob(filename, self.img_size)
    ...
    def read_blob(self, filename, img_size):
...
def multithread_dataloader(data_config):
    ...
    for name in data_config:
        ...
        if 'batch_size' in this_dataset_flag:
            ...
            batch_size = 1
        if 'img_size' in this_dataset_flag:
            img_size = this_dataset_flag['img_size']
        else:
            img_size = (640, 640)
        ...
        preload_data = PreloadData(path['image'], is_preload=is_preload, is_label=is_label, is_mask=is_mask, is_transfrom=is_transform, is_transform_in_gray=is_transform_in_gray, img_size=img_size)
